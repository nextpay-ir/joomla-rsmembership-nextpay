...
class Nextpay_Payment
{
    ...
    public $trans_id = "";
    public $params = array();
    public $server_soap = "http://api.nextpay.org/payment/token.wsdl";
    //public $server_soap = "http://api.nextpay.org/payment/token?wsdl";
    public $server_http = "http://api.nextpay.org/payment/token.http";
    public $request_http = "http://api.nextpay.org/payment/request.payment.do.http";
    public $request_verify_soap = "http://api.nextpay.org/payment/verify.wsdl";
    //public $request_verify_soap = "http://api.nextpay.org/payment/verify?wsdl";
    public $request_verify_http = "http://api.nextpay.org/payment/verify.http";
        ...
        switch ($this->default_verify)
        {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        if ($res != "" && $res != NULL) {
                            ...
                            if (strpos($res, ',') !== false)
                                $this->trans_id = explode(',', $res['TokenGeneratorResult'])[0];
                            else
                                $this->code_error($res);
                        }
                    }
            ...
            case Type_Verify::Http:
                try
                {
                    ...
                    if ($server_output != "" && $server_output != NULL) {

                        if(strlen($server_output) <= 4)
                            $this->code_error($server_output);

                        if (strpos($server_output, ',') !== false)
                            $this->trans_id = explode(',', $server_output)[0];
                        else
                            $this->code_error($server_output);
                    }
                }
     ...
     * @return int|mixed
     */
    public function PaymentVerifection($params=false, $api_key=false, $trans_id=false, $amount=false)
    {
        $res = 0;
        if(is_array($params))
        {
            ...
            case Type_Verify::NuSoap:
                try
                {
                    ...
                    else
                    {
                        ...
                        if ($res != "" && $res != NULL) {
                            return intval($res['PaymentVerifectionResult']);
                        }
                    }
                }
                catch(Exception $e){
